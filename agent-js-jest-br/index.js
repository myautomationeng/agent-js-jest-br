...
class JestReportPortal {
    ...
    onTestResult(test, testResult, aggregatedResults) {
        const lenght = testResult.testResults[0].ancestorTitles.length;

        if (lenght > 1) {
            const suiteName = testResult.testResults[0].ancestorTitles[0];
            this._startMainSuite(suiteName, test.path);
        }


        const suiteJson = testResult.testResults.reduce(function (r, a) {
            r[a.ancestorTitles[lenght - 1]] = r[a.ancestorTitles[lenght - 1]] || [];
            r[a.ancestorTitles[lenght - 1]].push(a);
            return r;
        }, Object.create(null));

        Object.keys(suiteJson).forEach((key) => {
            const suiteName = key;
            this._startSuite(suiteName, test.path);
            suiteJson[key].forEach(t => {
                if (!t.invocations) {
    ...
    _startMainSuite(suiteName, path) {
        ...
        promiseErrorHandler(promise);
        this.mainSuiteId = tempId;
    }

    _startSuite(suiteName, path) {
        const codeRef = getCodeRef(path, suiteName);
        const { tempId, promise } = this.client.startTestItem(getSuiteStartObject(suiteName, codeRef),
            this.tempLaunchId, this.mainSuiteId ? this.mainSuiteId : undefined);
    ...
    _finishMainSuite() {
        const { promise } = this.client.finishTestItem(this.mainSuiteId, {});
